...
/** Documents that contribute to RAG (ready chunks, not toggled off). */
async function getActiveRagDocumentIdSet() {
  await connectDB();
...
export async function searchSimilar(queryEmbedding, limit = 10, options = {}) {
  ...
  let chunks = await getCachedChunks(filter);
  if (!options.documentId) {
    const activeSet = await getActiveRagDocumentIdSet();
    chunks = chunks.filter((c) => activeSet.has(String(c.documentId)));
  }
...
export async function searchSimilarBatch(queryEmbeddings, limitPerQuery = 10, options = {}) {
  ...
  const restrictActive = !options.documentId;
  const activeSet = restrictActive ? await getActiveRagDocumentIdSet() : null;

  const trimToActive = (rows) => {
    if (!restrictActive || !activeSet) return rows;
    return rows.map((row) => {
      const filtered = (row || []).filter((c) => activeSet.has(String(c.documentId)));
      return filtered.slice(0, limitPerQuery);
    });
  };
  ...
  if (VECTOR_INDEX_NAME && process.env.MONGODB_URI?.includes('mongodb.net')) {
    try {
      const atlasLimit = restrictActive
        ? Math.min(Math.max(limitPerQuery * 10, 40), 200)
        : limitPerQuery;
      const results = await Promise.all(
        queryEmbeddings.map((emb) =>
          emb?.length ? searchSimilarAtlas(emb, atlasLimit, options) : []
        )
      );
      const trimmed = trimToActive(results);
      const totalFound = trimmed.reduce((sum, r) => sum + r.length, 0);
      if (totalFound === 0 && results.some((r) => r.length > 0)) {
        console.warn('[RAG] Atlas hits were all from inactive documents; falling back to in-memory');
