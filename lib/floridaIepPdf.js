...
const FLORIDA_IEP_LOGO_URL = '/uploads/profile/1_Logo.png';

async function loadFloridaHeaderLogo() {
  try {
    const res = await fetch(FLORIDA_IEP_LOGO_URL, { cache: 'force-cache' });
 ...
 * @param {{ stampDate?: Date|string, fileName?: string }} [options]
 */
export async function downloadFloridaIepPdf(student, editablePlan, options = {}) {
  ...
  const logo = await loadFloridaHeaderLogo();
