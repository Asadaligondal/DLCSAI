 ...
 */
async function buildFloridaIepPdf(student, editablePlan, options = {}) {
  ...
  const logo = await loadFloridaHeaderLogo(options.logoUrl);
      ...
      const primaryExc =
        ...
        '—';
      const excList = (student.disabilities || []).filter(Boolean).join(', ') || '—';
      const domainAreaList = Array.isArray(student.domainAreas) && student.domainAreas.length
        ? student.domainAreas.join(', ')
        : '—';
      ...
      y += rH;
      drawRow([{ text: `Case Manager: ${student.caseManager || '—'}`, w: cW }], y, rH);
      y += rH;
      drawRow(
        [
      ...
      y += rH;
      drawRow([{ text: `Exceptionalities: ${excList}`, w: cW }], y, rH);
      y += rH;
      drawRow([{ text: `Domain area(s): ${domainAreaList}`, w: cW }], y, rH);
      y += rH;
      ...
      y += rH;
      drawRow([{ text: `Associated Plan: ${student.associatedPlans || '—'}`, w: cW }], y, rH);
      y += rH + 6;
...
export async function downloadFloridaIepPdf(student, editablePlan, options = {}) {
  const pdf = await buildFloridaIepPdf(student, editablePlan, options);
...
/** Build the same Florida PDF in memory for in-browser preview (iframe / new tab). Caller must call revoke() when done. */
export async function getFloridaIepPdfBlobUrl(student, editablePlan, options = {}) {
  const pdf = await buildFloridaIepPdf(student, editablePlan, options);
