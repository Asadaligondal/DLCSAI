...
export function formatGenerationContextForPrompt(gc) {
  ...
  if (d && typeof d === 'object') {
    ...
    if (d.reviewDueDate) parts.push(`Review due date: ${d.reviewDueDate}`);
    ...
    if (d.durationDate) parts.push(`IEP duration / end date: ${d.durationDate}`);
    if (d.amendmentDate) parts.push(`Amendment date: ${d.amendmentDate}`);
    if (d.previouslyAmended) parts.push(`Previously amended: ${d.previouslyAmended}`);
  }
  return parts.join('\n');
}
