...
const LABEL_REMAP = {
  ...
  section_aligned: null // handled by direct section key matching
};
...
function getChunksByLabels(ragContextByQuery, labels, strategy = 'baseline') {
  ...
  if (strategy === 'section_aligned') {
    // For section_aligned, labels are passed as section keys directly — handled by caller
    effectiveLabels = labels.map(l => l.toLowerCase());
  } else if (LABEL_REMAP[strategy]) {
...
async function callSectionLLM(sectionKey, { studentProfile, ragContextByQuery, accommodationsList, customGoalsList, apiKey, strategy = 'baseline', sectionMetrics }) {
  ...
  // For section_aligned, match by section key; otherwise use contextLabels with remapping
  const lookupLabels = strategy === 'section_aligned' ? [sectionKey] : section.contextLabels;
