...
// ---------------------------------------------------------------------------
function buildStudentProfile({ studentGrade, studentAge, areaOfNeed, currentPerformance, disabilityCategory, instructionalSetting, generationContextExtra }) {
  const lines = [
  ...
  if (generationContextExtra && String(generationContextExtra).trim()) {
    lines.push('');
    lines.push('--- IEP meeting / generation context (use to align tone and priorities) ---');
    lines.push(String(generationContextExtra).trim());
  }
  return lines.join('\n');
}

/**
 * Format optional generation context from the M5 regenerate modal for prompts.
 */
export function formatGenerationContextForPrompt(gc) {
  if (!gc || typeof gc !== 'object') return '';
  const parts = [];
  if (gc.generationType) parts.push(`Meeting / generation type: ${gc.generationType}`);
  if (gc.meetingPurpose) parts.push(`Meeting purpose or focus: ${gc.meetingPurpose}`);
  const d = gc.datesSummary;
  if (d && typeof d === 'object') {
    if (d.originalMeetingPlanDate) parts.push(`Original meeting / plan date: ${d.originalMeetingPlanDate}`);
    if (d.reviewDueDate) parts.push(`Review / duration date: ${d.reviewDueDate}`);
    if (d.reevaluationDueDate) parts.push(`Reevaluation due date: ${d.reevaluationDueDate}`);
    if (d.initiationDate) parts.push(`IEP initiation date: ${d.initiationDate}`);
    if (d.durationDate && d.durationDate !== d.reviewDueDate) parts.push(`IEP duration / end date: ${d.durationDate}`);
    if (d.amendmentDate) parts.push(`Amendment date: ${d.amendmentDate}`);
  }
  return parts.join('\n');
}
...
2) MANDATORY GROUPS — The user message lists every exceptionality that MUST appear as its own object in annualGoalsByExceptionality (exact "exceptionality" string). Include ALL of them. If the goal bank has little or no text for one label, still add that group with at least one appropriate annual goal consistent with that disability and the same writing style as the bank; use related strands from the context when helpful.
3) Do NOT paraphrase into vague language. Keep the specific observable actions from the source.
4) Do NOT add percentages, trial counts, or measurement windows unless they appear in the context.
5) Never output raw templates with blanks like "________". Convert to clean professional sentences.
6) Each exceptionality should have multiple goals covering DIFFERENT domains when the context supports it (e.g., expressive language, receptive language, articulation, fine motor, visual motor).
7) Reference provided accommodations as conditions (e.g., "Given text-to-speech...") when relevant.
...
Output ONLY valid JSON:
{
  ...
  "shortTermObjectivesByExceptionality": [{"exceptionality": "Name", "objectives": [{"referenceId": "0", "objective": "Objective text", "alignedAnnualGoalReferenceId": "0"}]}]
    ...
    buildUserPrompt({ studentProfile, contextText, accommodationsList }) {
      const dm = studentProfile.match(/Disability Category:\s*([^\n]+)/i);
      const raw = dm ? dm[1].trim() : '';
      const requiredLabels = raw && raw !== 'Not specified'
        ? raw.split(',').map((s) => s.trim()).filter(Boolean)
        : [];
      const requiredBlock = requiredLabels.length
        ? [
          '',
          'REQUIRED EXCEPTIONALITY GROUPS (output one annualGoalsByExceptionality entry per line — use this exact "exceptionality" string):',
          ...requiredLabels.map((label, i) => `${i + 1}. ${label}`)
        ].join('\n')
        : '';
      return [
        studentProfile,
        ...
        '',
        'Generate goals and objectives for EACH required exceptionality above. Return valid JSON only.'
      ].join('\n');
    },
...
Output ONLY valid JSON:
{
  ...
  "academicPerformanceAchievement": "Detailed academic achievement paragraph..."
    ...
    buildUserPrompt({ studentProfile, contextText, assessmentContext }) {
      const ac = assessmentContext && String(assessmentContext).trim();
      return [
        studentProfile,
        '',
        ...(ac
          ? [
              'ASSESSMENT CONTEXT (formal or informal assessments — use for PLAAFP and academic achievement detail):',
              ac,
              ''
            ]
          : []),
...
// ---------------------------------------------------------------------------
async function callSectionLLM(sectionKey, { studentProfile, ragContextByQuery, accommodationsList, customGoalsList, apiKey, strategy = 'baseline', sectionMetrics, assessmentContext }) {
...
export async function generateIEPParallel({
  ...
  const generationContextExtra = formatGenerationContextForPrompt(generationContext);
  ...
  const acTrim = assessmentContext && String(assessmentContext).trim() ? String(assessmentContext).trim() : '';
  const sharedArgs = { studentProfile, ragContextByQuery, accommodationsList, customGoalsList, apiKey, strategy, sectionMetrics, assessmentContext: acTrim };
