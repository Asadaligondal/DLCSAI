...
export async function getRagContext(studentContext) {
  try {
    ...
    for (let i = 0; i < embeddings.length; i++) {
      const chunks = await searchSimilar(embeddings[i], TOP_K_PER_QUERY);
      allChunks.push(...chunks.map(c => ({ ...c, queryIdx: i, query: queries[i] })));
    }
    ...
    final.forEach((c, i) => {
      console.log(`[RAG] --- Chunk ${i + 1} (score: ${c.score.toFixed(3)}, from query ${c.queryIdx + 1}) ---`);
